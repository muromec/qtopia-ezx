...
public:
            ...
            : QWidget(0, Qt::FramelessWindowHint), frames(0), _animOffsetColor(0), menuItem(0), menuPos(0, 0), menuDestPos(0, 0)
	{
            ...
            aNot = new GfxImageRef(notSupported);

            {
                ...
                menuHighlight = new GfxImageRef(high);
            }
   ...
   //             printf("%x\n", _offset);
        ...
        void go(GfxPainter &p)
        {
            ...
            if(_imgState == Tint && _animOffset < (_imgOffset + 12)) {
                ...
                p.drawImage(r.topLeft(), _backImg);
                ++_animOffsetColor;
...
//            p.copy(_backImg.rect(), _backImg);
            ...
            for(int ii = 0; ii < names.count(); ++ii) {
                ...
                // p.blit(imgRect, img);
                p.drawImage(imgRect.topLeft(), *gnames.at(ii));
            }
            ...
            if(0.0f != fade) {
                ...
                p.drawImage(blurrect.topLeft(), *aNot);
                ...
                p.drawImage(2, blurrect.y() + 2 + menuPos.y(), *menuHighlight);
                for(int ii = 0; ii < menuItems.count(); ++ii) {
                    GfxImageRef *img = menuItems.at(ii);
            ...
            p.setOpacity(1.);
            p.drawImage(240 - fpsImage.width() - 10, 320 - fpsImage.height() - 10, fpsImage);
...
private:
        QList<QImage> names;
        QList<GfxImageRef *> gnames;
        QList<GfxImageRef *> menuItems;
        int menuItem;
        QPoint menuPos;
        QPoint menuDestPos;
        QImage fpsImage;

        GfxImageRef *menuHighlight;
        QImage notSupported;
        QImage _backImg;
        GfxImageRef *aNot;
