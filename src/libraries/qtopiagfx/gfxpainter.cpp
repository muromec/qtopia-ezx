...
#if BUFFER_BPP == 18
        buffer = new uchar[width * height * 3];
        pImg = new QImage(buffer, width, height, QImage::Format_RGB666);
#else
...
#if BUFFER_BPP == 18
    uchar *dest = buffer;
    int val18 = (val & 0x07ff) << 1 + (val &0xf800)<<2;
    for(int ii = 0; ii < height; ++ii) {
        q_memoryroutines.memset_24(dest, val18, width);
        dest += step*3;
    }
#else
...
{
#if BUFFER_BPP == 18
    if(useQt || _opacity != 0xFF || img.format() != QImage::Format_RGB666)  {
#else
...
GfxImageRef GfxPainter::imgRef(const QRect &r) const
{
    ...
    if(depth == Depth_16) {
        ...
        return GfxImageRef(buffer + step * 3 * imgRect.y() + imgRect.x() * 3,
                      imgRect.width(), imgRect.height(),
                      step * 3, QImage::Format_RGB666);
    } else {
...
QImage GfxPainter::img(const QRect &r) const
{
    ...
    if(depth == Depth_16) {
        ...
        return QImage(buffer + step * 3 * imgRect.y() + imgRect.x() * 3,
                      imgRect.width(), imgRect.height(),
                      step * 3, QImage::Format_RGB666);
    } else {
