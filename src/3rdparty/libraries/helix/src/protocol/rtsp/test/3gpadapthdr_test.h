...
#ifndef _3GPADAPTHDR_TEST_H
#define _3GPADAPTHDR_TEST_H

#include "hx_cmd_based_test.h"
#include "3gpadapthdr.h"

class C3gpAdaptationHeaderTest : public HLXCmdBasedTest
{
public:
    C3gpAdaptationHeaderTest();
    ~C3gpAdaptationHeaderTest();
...
protected:
    bool HandleConstructCmd(const UTVector<UTString>& info);
    bool HandleGetValuesCmd(const UTVector<UTString>& info);
    ...
    bool HandleSetULONG32PropCmd(const UTVector<UTString>& info);
    bool HandleGetStringCmd(const UTVector<UTString>& info);

private:
    C3gpAdaptationHeader* GetHdr(const UTString& name);

    C3gpAdaptationHeader* m_pA;
    C3gpAdaptationHeader* m_pB;
    IHXValues* m_pValues;
    IUnknown* m_pContext;
};

#endif // _3GPADAPTHDR_TEST_H
