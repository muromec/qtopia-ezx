/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: rtsppars.h,v 1.6 2007/07/06 20:51:36 jfinnecy Exp $
 * 
 ...
 * 
 * Purpose:
 *	Transform RTSP/1.0 data into RTSPMessage objects. 
 *
 */
...
public:
    ...
    RTSPMessage* parse(const char* pMsg, UINT32& nMsgLen);
...
private:
    ...
    int parseRangeValue(const char* pValue, MIMEHeader* pHeader);
    int parseAuthenticationValue(const char* pValue, MIMEHeader* pHeader);
    ...
    const char* NextRTPInfoEntry(const char* pValue, const char* pTarget, const char cDelimiter);
    ...
    UINT32 scanMessageHeader(const char* pMsg, UINT32 nMsgLen);

    RTSPMessage* parseResponse();
    RTSPResponseMessage* parseResponseLine(CHXString& str);

    RTSPMessage* parseRequest();
    RTSPRequestMessage* parseRequestLine(CHXString& str);
