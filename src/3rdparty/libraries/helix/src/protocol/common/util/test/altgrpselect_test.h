/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: altgrpselect_test.h,v 1.2 2004/08/04 16:45:30 acolwell Exp $
 * 
...
class CHXAltGroupSelectorTest : public HLXCmdBasedTest
{
public:
    CHXAltGroupSelectorTest();
    ~CHXAltGroupSelectorTest();
...
protected:
    ...
    bool HandleSelectCmd(const UTVector<UTString>& info);
    bool HandleAddMaxBWFilterCmd(const UTVector<UTString>& info);
    bool HandleAddLanguageFilterCmd(const UTVector<UTString>& info);
    bool HandleAddHighestBWFilterCmd(const UTVector<UTString>& info);
    bool HandleAddMimetypeFilterCmd(const UTVector<UTString>& info);

    bool HandleSetMimetypesCmd(const UTVector<UTString>& info);
...
private:
    ...
    CHXTestPluginHdlr* m_pPluginHdlr;
    SDPMediaDescParser* m_pParser;
    CHXAltHdrTestHelper m_altTestHlpr;
    CHXAltGroupSelector m_selector;
    IHXValues* m_pA;
    IHXValues* m_pB;
    IHXList* m_pList;
