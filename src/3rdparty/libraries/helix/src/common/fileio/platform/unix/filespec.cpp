/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: filespec.cpp,v 1.7 2005/10/05 23:50:51 rggammon Exp $
 * 
...
CHXFileSpecifier CHXDirSpecifier::SpecifyChildFile(const char *child) const
{
    if(IsSet())
    {
        const int nOSSeparatorStringLen = sizeof(OS_SEPARATOR_STRING) - 1;
        const int nPathLen = m_parser.m_path.GetLength();
        
        const char* pPath = (const char*)m_parser.m_path;
	const char* pPos = pPath + nPathLen - nOSSeparatorStringLen;

        if( (strncmp(OS_SEPARATOR_STRING, pPos, nOSSeparatorStringLen) == 0)
#ifdef _WINDOWS
            || (*pPos == '/') // Also recognize '/' on windows
#endif
           )
        {
        ...
        {
	    return CHXFileSpecifier( m_parser.m_path + OS_SEPARATOR_STRING + child );
        }
    }
    else
	return child;
...
CHXDirSpecifier CHXDirSpecifier::SpecifyChildDirectory(const char *child) const
{
	...
	const char* pPos = pPath + nPathLen - nOSSeparatorStringLen;
        ...
        {
	    return CHXDirSpecifier( m_parser.m_path + OS_SEPARATOR_STRING + child );
        }
    }
    else
	return child;
