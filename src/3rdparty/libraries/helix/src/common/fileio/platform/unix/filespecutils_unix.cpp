/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: filespecutils_unix.cpp,v 1.5 2007/07/06 20:35:16 jfinnecy Exp $
 * 
        ...
        const CHXDirSpecifier* pNewDirectoryIfAny)
{
        ...
        fileSpecNew = destDir.SpecifyChildFile(strNewName);

        nResult = rename(fileSpec.GetPathName(), fileSpecNew.GetPathName());
        if(nResult == 0)
        {
            res = HXR_OK;
        }
