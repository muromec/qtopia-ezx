/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxsbuffer.cpp,v 1.5 2007/07/06 20:34:58 jfinnecy Exp $
 * 
                                ...
                                UINT32 ulLen, REF(IHXBuffer*) rpBuffer)
{
   ...
   if (pBuffer)
   {
#if defined(HELIX_FEATURE_USE_NULL_STATIC_BUFFER)
	CHXStaticBufferNULL* pObj = new CHXStaticBufferNULL(pBuffer,ulOffset, ulLen);
#else
	CHXStaticBuffer* pObj = new CHXStaticBuffer(pBuffer, ulOffset,ulLen);
#endif
	if (pObj)
	{
	    retVal = pObj->QueryInterface(IID_IHXBuffer, (void**)&rpBuffer);
	}
	else
	{
