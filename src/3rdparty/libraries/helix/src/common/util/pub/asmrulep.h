/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: asmrulep.h,v 1.10 2007/07/06 20:39:23 jfinnecy Exp $
 * 
...
public:
		    	...
		    	ASMRuleBook(IUnknown* pContext, const char* pRuleBook);
		    	~ASMRuleBook();
    HX_RESULT           GetProperties(UINT16 unRuleNumber,
			    IHXValues*& pRuleProps);
    HX_RESULT           GetSubscription(HXBOOL* pSubInfo, IHXValues* pVariables);
    HX_RESULT		FindVariable(HXBOOL* pFound, const char* pVariable);
    HX_RESULT		FindProperty(HXBOOL* pFound, const char* pProperty);
    UINT16              GetNumRules() { return m_unNumRules; }
    UINT32              GetNumThresholds() { return m_ulNumThresholds; }
    HXBOOL		HasExpression();
    HXBOOL		CheckCurrentRangeEmpty( int nRuleNumber );
...
private:
    void		Init(const char* pRuleBook);
    HX_RESULT           InitRulesArray();
    ASMRule*            m_pRules;
    UINT16		m_unNumRules;
    UINT32		m_ulNumThresholds;
    HXBOOL *              m_pValidRulesArray;
    HXBOOL *              m_pDeletedRulesArray;
    IUnknown*		m_pContext;
    char *        m_pRuleBook;
};
