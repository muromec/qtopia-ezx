/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: 444v565.c,v 1.5 2007/07/06 20:53:52 jfinnecy Exp $
 * 
...
#define DITHER_MAGIC 0xc0380700


/* RGB444 version: */
static void dblineXRGBtoRGB444v565x2 (unsigned char *d1, unsigned char *d2, int dest_x,
 ...
 */
int I420toRGB444v565 (unsigned char *dest_ptr, int dest_width, int dest_height,
    int dest_pitch, int dest_x, int dest_y, int dest_dx, int dest_dy,
    unsigned char *src_ptr, int src_width, int src_height, int src_pitch,
    int src_x, int src_y, int src_dx, int src_dy,
		      color_data_t* pcd)
{
    ...
    if (scale_x == 2 && scale_y == 2) {
        ...
        void (*x2) (unsigned char *d1, unsigned char *d2, int dest_x,
            unsigned char *s1, unsigned char *s2, int src_x, int dx, color_data_t* pcd) = dblineXRGBtoRGB444v565x2;
    ...
    if (scale_x == 0 && scale_y == 0) {
        ...
        for (j = 0; j < dest_dy>>1; j ++) {
            ...
            /* convert two lines a time: */
            halfI420toRGB444v565 (d1, dest_x, sy1, sy2, su, sv, src_x, src_dx, 0, pcd);
            sy1 += src_pitch*2; sy2 += src_pitch*2;
            su  += src_pitch/2; sv  += src_pitch/2;
            d1  += dest_pitch;

            halfI420toRGB444v565 (d1, dest_x, sy1, sy2, su, sv, src_x, src_dx, 1, pcd);
            sy1 += src_pitch*2; sy2 += src_pitch*2;
            su  += src_pitch/2; sv  += src_pitch/2;
            d1  += dest_pitch;
        }
...
/*
 * InitI420toRGB444v565 initializes the minimum set of clip tables necessary to perform
 * color conversion to 12 bit RGB packed into a 565 value.  The 8-bit clip table is 
 * necessary for 2x interpolation.
 *
 * This routine also calls SetSrcI420Colors, which initializes color conversion tables
 * and adds color balance.
 */
void InitI420toRGB444v565(float brightness, float contrast, float saturation, float hue, 
			  color_data_t* pcd)
{
    InitColorData(pcd);
