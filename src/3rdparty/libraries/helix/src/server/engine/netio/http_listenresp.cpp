...
HTTPListenResponse::HTTPListenResponse(Process* _proc) :
    ListenResponse(_proc)
{
    // Empty
}

HTTPListenResponse::~HTTPListenResponse(void)
{
    HX_RELEASE(m_pSock);
}

HX_RESULT
HTTPListenResponse::Init(UINT16 uPort)
{
    HX_RESULT rc;
    ...
    if (SUCCEEDED(rc))
    {
        g_pProtMgr->SetDefaultProtocol(HXPROT_HTTP, m_pAddr);
    }
...
STDMETHODIMP
HTTPListenResponse::OnError(HX_RESULT err)
{
    //Remove the assert
