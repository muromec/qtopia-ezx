...
{
    return m_pMgr->ScheduleSinkNotifications(pClient, pSession, nEvent, m_pProc);
}
...
, m_pMutex(NULL)
{
    ...
    m_pClientTable = new CHXID(10000);
...
ClientStatsManager::AddClient(IHXClientStats* pClient, Process* pProc)
{
    ...
    ScheduleSinkNotifications(pClient, NULL, CSEVENT_CLIENT_CONNECT, pProc);
...
// We only verify this in debug builds!
    ...
    ScheduleSinkNotifications(pClient, NULL, CSEVENT_CLIENT_DISCONNECT, pProc);
...
ClientStatsManager::SessionDone(IHXClientStats* pClient,
                                ...
                                Process* pProc)
{
    ScheduleSinkNotifications(pClient, pSession, CSEVENT_SESSION_DONE, pProc);
...
ClientStatsTimer::Func()
{
    ...
    if (FAILED(pCB->Init(m_pStats, NULL, m_pSink, m_pSinkProc, CSEVENT_TIMER)))
    {
        HX_DELETE(pCB);
        return HXR_FAIL;
    }
