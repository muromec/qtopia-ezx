/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: h261dec.c,v 1.6 2007/07/06 22:00:33 jfinnecy Exp $
 * 
...
#define IFRAME      3
{
    ...
    while (1) {     // Process until next to last startcode
        ...
        if (structStatus == OK) {
            ...
            s->status.dwGobs += pGob->num_mb / HX_MAX(1,pGob->mb_width);
    ...
    *structStatus = doGobStructures( s, gob, s->mb, &start[0], &gn[0] );
    if (*structStatus == OK  &&  gn[0] == s->next_gob) {
        ...
        *structStatus = H261_ERROR;
        ...
        ++s->status.dwBadgobs;
        s->status.dwGobs += gob->num_mb / HX_MAX(1,gob->mb_width);
...
static void gobConcealment( H261Decoder *s, GOB_DESCR *gob, MACROBLOCK_DESCR mb[] )
{
    ...
    if (s->status.dwUpdateGobs == 0)   // If this is the first bad gob
    {
        ...
        s->status.dwUpdateGobs = gob->num_mb/HX_MAX(1,gob->mb_width);
    } else {
        s->status.dwUpdateGobs = s->next_gob + gob->num_mb / HX_MAX(1,gob->mb_width) -
                     ...
                     BS_PTR * start )   // Returns position for bit after startcode
{
    ...
    zeros = trailing[b];
//    nbytes = (numbits + bs->bitptr - 8 - HX_MAX(H263_GN_BITS, H261_GN_BITS) ) >> 3;
...
static int DrawVector( unsigned char data[], int xdim, int x1, int y1, int x2, int y2 )
{
    ...
    if (y1 == y2) {         // Draw horizontal line
        data += y1 * xdim;
        for (x = HX_MIN(x1,x2); x <= HX_MAX(x1,x2); ++x)  markPixel( &data[x] );
    } else if (x1 == x2) {  // Draw vertical line
        data += HX_MIN(y1,y2) * xdim;
        for (y = HX_MIN(y1,y2); y <= HX_MAX(y1,y2); ++y) {
            markPixel( &data[x1] );
            data += xdim;
        }
