/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: mcomp.c,v 1.6 2007/07/06 22:00:33 jfinnecy Exp $
 * 
                            ...
                            )
{
    ...
    if (mb->mtype == MTYPE263_INTER4V) {
        for (blk = 0; blk < 4; ++blk) { // Treat each 8x8 block separately
            ...
            firstCol = HX_MAX( firstCol,     (-mb->blkMvBx[blk] + 1) >> 1 );
            lastCol  = HX_MIN( lastCol, 15 - ((mb->blkMvBx[blk] + 1) >> 1));
            firstRow = HX_MAX( firstRow,     (-mb->blkMvBy[blk] + 1) >> 1 );
            lastRow  = HX_MIN( lastRow, 15 - ((mb->blkMvBy[blk] + 1) >> 1));
        ...
        // Only use pixels in current MB for backwards prediction
        firstCol = HX_MAX( 0,       (-mb->blkMvBx[0] + 1) >> 1 );
        lastCol  = HX_MIN( 15, 15 - ((mb->blkMvBx[0] + 1) >> 1));
        firstRow = HX_MAX( 0,       (-mb->blkMvBy[0] + 1) >> 1 );
        lastRow  = HX_MIN( 15, 15 - ((mb->blkMvBy[0] + 1) >> 1));
//#define MMX_AVERAGE_TEST01
    ...
    hdim = pic->cb.hoffset;
    // Only use pixels in current MB for backwards prediction
    firstCol = HX_MAX( 0,     (-cX + 1) >> 1 );
    lastCol  = HX_MIN( 7, 7 - ((cX + 1) >> 1));
    firstRow = HX_MAX( 0,     (-cY + 1) >> 1 );
    lastRow  = HX_MIN( 7, 7 - ((cY + 1) >> 1));
