      ...
      enum RXFrameType frame_type, Word32 synth[], Word32 A_t[] )
{
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      if ( overflow ) {
         ...
         */
      //XXXctd memcpy changed to memmove because of Openwave assertion,
      // that prohibits memcpy src and dest overlap. This conforms with ISO C standards.
      memmove( &st->old_exc[0], &st->old_exc[L_SUBFR], ( PIT_MAX + L_INTERPOL ) << 2);
