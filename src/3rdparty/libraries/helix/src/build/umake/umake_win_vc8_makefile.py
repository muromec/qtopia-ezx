...
class Workspace(Workspace):
    ...
    def generate(self):
        text = []
        text.append('Microsoft Visual Studio Solution File, Format Version 9.00')
        text.append('# Visual Studio 2005')
        ...
        text.append('\t\tBuild|Win32 = Build|Win32')
        text.append('\tEndGlobalSection')

        text.append('\tGlobalSection(ProjectConfigurationPlatforms) = postSolution')
        for (name, path) in self.projects:
            guid = make_guid(name)
            text.append('\t\t%s.Build|Win32.ActiveCfg = Build|Win32' % (guid))
            text.append('\t\t%s.Build|Win32.Build.0 = Build|Win32' % (guid))
        text.append('\tEndGlobalSection')

        text.append('\tGlobalSection(SolutionProperties) = preSolution')
        text.append('\t\tHideSolutionNode = FALSE')
        text.append('\tEndGlobalSection')
