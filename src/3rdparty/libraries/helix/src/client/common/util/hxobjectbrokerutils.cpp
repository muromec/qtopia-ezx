...
namespace HXObjectBrokerUtils
{
HX_RESULT SetupDllAccessPaths(const char* pRootDirectory)
{
    CHXDirSpecifier rootDirectorySpec;

    if (pRootDirectory)
    {
        rootDirectorySpec = pRootDirectory;
    }
    else
    {
...
HX_RESULT AddPluginPath(const char* pName, const char* pPath)
{
    if (!pPath || !*pPath)
    {
    ...
    CHXPluginHelper pluginHelper;
    return pluginHelper.AddMountPoint(pName, pPath);
}

HX_RESULT InitObjectBroker(IUnknown** ppUnkObjectBroker)
{
    if (!ppUnkObjectBroker)
    {
        return HXR_INVALID_PARAMETER;
    }
    ...
    HX_RESULT res = HXObjBrokerHelper::Instance().InitObjectBroker(spUnk.AsInOutParam());
    if (SUCCEEDED(res))
    {
        spUnk.AsPtr(ppUnkObjectBroker);
    }

    return res;
}

void TerminateObjectBroker()
{
    HXObjBrokerHelper::Instance().TerminateObjectBroker();
}
}
