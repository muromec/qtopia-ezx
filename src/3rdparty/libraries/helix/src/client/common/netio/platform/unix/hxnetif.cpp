/* ***** BEGIN LICENSE BLOCK *****
 * Source last modified: $Id: hxnetif.cpp,v 1.12 2005/03/14 20:28:51 bobclark Exp $
 * 
    ...
    for (i = 0, ifr = ifc.ifc_req; i < num; i++, ifr += 1) 
    {
        if (-1 != ioctl(fd, SIOCGIFFLAGS, ifr)) 
        {
            ...
            if (-1 != ioctl(fd, SIOCGIFADDR, ifr))
            {
                ...
                const struct sockaddr_in* psa = ((struct sockaddr_in *)&ifr->ifr_broadaddr);
                const UINT8* pin = (const UINT8*)&psa->sin_addr.s_addr;

                sprintf(szValue, "%u.%u.%u.%u", 
